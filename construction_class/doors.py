...
class Door(BaseElement):
    ...
    def get_area(self) -> float:
        ...
        sum_area = area * self.count
        return sum_area

class Doors(BaseConstruction):
    def __init__(self):
        super().__init__()
        self.add_door(r=0.0, area=0.0, size='0*0')

    def add_door(self, r: float = 0.0, area: float = 0.0, size: str = '0*0', index=0):
        elem = Door()
        elem.r_pr = r
        elem.area = area
        elem.set_size(size)
    ...
    def get_sum_area(self) -> dict:
        """Расчет суммарной площади дверей отдельно по азимутам"""
        sum_area = 0.0
        ...
        if len(self.elements) > 0:
            ...
            for i, elem in enumerate(self.elements):
                if type(elem) is Door:
                    ...
                    # добавление элемента с площадью
                    ...
                    if cell_item:
                        cell_item.setFlags(cell_item.flags() ^ QtCore.Qt.ItemIsEditable)
                    # добавление элемента с размерами
                    ...
                    if cell_item:
                        cell_item.setFlags(cell_item.flags() ^ QtCore.Qt.ItemIsEditable)
                    # добавление количество элементов
                    el = QTableWidgetItem(str(elem.count))
                    table.setItem(i, 6, el)
    ...
    def get_text_result(self):
        ...
        s = f'<p>Суммарная площадь дверей S = {self.get_sum_area()} м².</p>'
        s += f'<p>Приведенное сопротивление теплопередаче дверей: Rпр = {self.r_pr} м²·ºС/Вт.</p>'
        s += "<p>В соответствии с п. 5.1 СП 50.13330.2012 сопротивление теплопередаче конструкции должно быть не ниже "
        s += 'требуемого сопротивления теплопередаче. </p>'
        if self.r_pr >= self.r_tr:
            ...
            s_usl = 'не '
            s_znak = '<'
        s += f'<p>Так как для конструкции выполняется условие, '
        s += f'а именно {self.r_pr} м²·ºС/Вт {s_znak} {self.r_tr} м²·ºС/Вт, следовательно, требование п. 5.1 ''' \
             f'<b>{s_usl} выполняется</b>.</p>'
        return s
