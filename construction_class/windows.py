...
class Windows(BaseConstruction):
    ...
    def get_text_result(self):
        ...
        s = '<p>Площадь светопрозрачного заполнения проемов раздельно по азимутам:</p>'
        area = self.get_sum_area()
        for key in area:
            if area[key] > 0:
                s += f'<p>- азимут {key} - S = {area[key]} м²;</p>'
        s += f'<p>Приведенное сопротивление теплопередаче конструкции: Rпр = {self.r_pr} м²·ºС/Вт.</p>'
